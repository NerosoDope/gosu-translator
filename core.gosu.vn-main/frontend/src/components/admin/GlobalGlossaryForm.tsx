...
import { useCreateGlobal_glossary, useUpdateGlobal_glossary } from '@/hooks/useGlobal_glossary';
import { gameCategoryAPI, languageAPI } from '@/lib/api'; // Import languageAPI
...
interface GlobalGlossaryFormProps {
  item?: GlobalGlossaryItem;
  onSuccess: () => void;
  onCancel: () => void;
}

export default function GlobalGlossaryForm({ item, onSuccess, onCancel }: GlobalGlossaryFormProps) {
  ...
  // Hooks
  const createGlobalGlossary = useCreateGlobal_glossary();
  const updateGlobalGlossary = useUpdateGlobal_glossary();
  ...
  useEffect(() => {
    const loadGameCategories = async () => {
      try {
        ...
        console.log('API response for game categories:', result);
        setGameCategories(result.data.items || []);
        console.log('Game categories loaded into state:', result.data.items || []);
      } catch (error) {
        console.error('Failed to load game categories in GlobalGlossaryForm:', error);
      }
    };
    loadGameCategories();
  }, []);
  ...
  useEffect(() => {
    const loadLanguages = async () => {
      try {
        ...
        console.error('Failed to load languages in GlobalGlossaryForm:', error);
      }
    };
    loadLanguages();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isTermValid || !isTranslatedTermValid || !isLanguagePairValid || !isGameCategoryValid) {
      return;
    }
    ...
    try {
      ...
      if (item?.id) {
        await updateGlobalGlossary.mutateAsync({ id: item.id, data: formData });
        toast.success('Cập nhật thuật ngữ thành công!');
      } else {
        await createGlobalGlossary.mutateAsync(formData);
        toast.success('Tạo thuật ngữ mới thành công!');
      }
      onSuccess();
    } catch (error: any) {
      ...
      let errorMessage = 'Không thể tạo/cập nhật thuật ngữ. Vui lòng thử lại.';
      ...
      if (!errorMessage || errorMessage === 'Không thể tạo/cập nhật thuật ngữ. Vui lòng thử lại.') {
  ...
  const isLoading = createGlobalGlossary.isLoading || updateGlobalGlossary.isLoading;
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="lg:sticky lg:top-6">
        {(term && translatedTerm && sourceLanguage && targetLanguage && !termError && !translatedTermError && !languagePairError && !gameCategoryError) ? (
          <div className="bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 dark:from-green-900/20 dark:via-emerald-900/20 dark:to-teal-900/20 rounded-xl p-6 border border-green-200 dark:border-green-800 shadow-lg">
            ...
            <div className="mt-4 p-3 bg-white dark:bg-gray-800 rounded-lg border border-green-200 dark:border-green-700">
              <p className="text-xs text-green-700 dark:text-green-300 text-center">
                Thuật ngữ này sẽ được sử dụng trong từ điển toàn cục của hệ thống dịch thuật
              </p>
            </div>
          </div>
        ) : (
