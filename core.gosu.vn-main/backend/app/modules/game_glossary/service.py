...
class Game_GlossaryService:
    ...
    async def find_translation(self, term: str, language_pair: str, game_id: Optional[int] = None) -> Optional[str]:
        """Tìm bản dịch theo term và language_pair (exact match). Lọc theo game_id nếu có."""
        return await self.repo.find_translation(term=term, language_pair=language_pair, game_id=game_id)
