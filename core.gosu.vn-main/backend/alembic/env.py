...
def run_migrations_offline() -> None:
    ...
    # Check if we're running inside a container by looking for specific environment
    import os
    if hostname == "postgres" and not os.path.exists('/.dockerenv'):
        hostname = "localhost"
    sync_url = f"postgresql+psycopg2://{parsed.username}:{encoded_password}@{hostname}:{parsed.port or 5432}{parsed.path}"
